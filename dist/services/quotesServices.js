...
class QuoteService {
    ...
    async getQuotes(regionRaw) {
        ...
        const latest = await prisma_1.prisma.quote.findFirst({
            where: { region },
        ...
        const fetched = await Promise.all(Object.entries(urls).map(([sourceKey, url]) => this.fetchOne(url, sourceKey, region)));
        const valid = fetched.filter((q) => q.buy_price > 0 && q.sell_price > 0);
        if (valid.length) {
            await Promise.all(valid.map((q) => prisma_1.prisma.quote.upsert({
                where: {
                    region_source: {
                        region,
                        source: q.source,
    ...
    async readFromDb(region) {
        const rows = await prisma_1.prisma.quote.findMany({
            where: { region },
            orderBy: { source: "asc" },
        });
        return rows.map((r) => ({
    ...
    async fetchOne(url, sourceKey, region) {
        try {
    ...
    async fetchAmbito(url, sourceKey, region) {
    ...
    async fetchDolarHoy(url, sourceKey, region) {
        ...
        $(".tile.is-parent.is-8 .tile.is-child").each((_i, el) => {
            ...
            if (topic.includes("compra"))
                buy = value;
            if (topic.includes("venta"))
                sell = value;
        });
    ...
    async fetchCronista(url, sourceKey, region) {
        const { data } = await axios_1.default.get(url, { timeout: 10000 });
        const $ = cheerio.load(data);
    ...
    async fetchWise(url, sourceKey, region) {
        const { data } = await axios_1.default.get(url, {
        ...
        return this.buildQuote(parseFloat(buy_price.toFixed(6)), parseFloat(sell_price.toFixed(6)), sourceKey, region, undefined, spread);
    }
    async fetchNubank(url, sourceKey, region) {
        const { data } = await axios_1.default.get(url, { timeout: 10000 });
        const $ = cheerio.load(data);
    ...
    async fetchNomad(url, sourceKey, region) {
        const { data } = await axios_1.default.get(url, {
